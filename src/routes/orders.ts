...
router.post("/add", async (req: Request, res: Response) => {
    try {
        ...
        if (!checkBody(req.body, ["productID", "buyerUsername", "sellerUsername", "totalPrice", "sellerPrice", "paymentMethod"])) {
            res.status(400).json({ result: false, error: "Missing or empty fields" });
            return;
        } else {
            // On vérifie si les utilisateurs existent
            const buyer: IUser | null = await User.findOne({ username: req.body.buyerUsername });
            const seller: IUser | null = await User.findOne({ username: req.body.sellerUsername });
            if (!buyer || !seller) {
...
router.put("/productSent", async (req : Request, res : Response) => {
    try {
        ...
        if (!checkBody(req.body, ["productID", "userToken"])) {
            ...
            const user : IUser | null = await User.findOne({token : req.body.userToken})
            if (!user){
                res.status(400).json({ result: false, error: "User not found" });
                return;
            } else {
                const order : IOrder | null = await Order.findOne({product : req.body.productID, seller : user._id})
                if (!order){
                    res.status(400).json({ result: false, error: "Product not found" });
                    return;
                } else {
    ...
    try {
        ...
        if (!checkBody(req.body, ["productID", "userToken"])) {
            ...
            if (!user){
                ...
                const order : IOrder | null = await Order.findOne({product : req.body.productID, buyer : user._id})
                if (!order){
                    res.status(400).json({ result: false, error: "Product not found" });
                    return;
                } else {
